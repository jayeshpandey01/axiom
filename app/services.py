...
from app.schemas import ScanCreate, TargetCreate
...
def queue_scan(db: Session, payload: ScanCreate, idempotency_key: str | None) -> ScanJob:
    if idempotency_key:
    ...
    job = ScanJob(**payload.model_dump(), idempotency_key=idempotency_key)
    db.add(job)
    db.commit()
    db.refresh(job)
    return job
...
def complete_scan(db: Session, job: ScanJob, summary: dict, raw_artifact: bytes | None = None) -> ScanResult:
    return persist_completed_result(db, job=job, summary=summary, raw_artifact=raw_artifact)
